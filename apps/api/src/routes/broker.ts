...
import pg from "pg";
const { Pool } = pg;
const pool = new Pool({ user: "postgres", host: "localhost", database: "easyfinder", password: "", port: 5432 });
...
async function getInventoryContext(): Promise<string> {
  try {
    const result = await pool.query("SELECT id::text, equipment, price::float, market_value::float, hours::float, score::float, state, source, operable, category, created_at FROM listings ORDER BY score DESC LIMIT 50");
    const dbRows = result.rows.length > 0 ? result.rows : null;
